...
// رابط تطبيق الويب لجلب سجلات الحضور الحية
const GAS_API_URL = "https://script.google.com/macros/s/AKfycbywiMWT0rfB3NybGp7NYdhRhRoP7jcqwYI4IY3GqCyaj398dcIBfJMAPvlfhf8ahW3bYA/exec";

// قاعدة بيانات الأعضاء بالأسماء الثلاثية
...
let groupedAttendanceData = {}; // خزان ذكي لتصنيف البيانات حسب الشهر والسنة

// دوال مساعدة وتنظيف نصوص
...
function goBackToDashboard() {
    const views = ['adminScreen', 'activitiesScreen', 'socialScreen', 'adminKaderListScreen', 'attendanceLogsScreen'];
    views.forEach(v => document.getElementById(v).classList.add('hidden'));
    document.getElementById('dashboardScreen').classList.remove('hidden');
}

// دوال البوابة السرية والإدارة
...
// -------------------------------------------------------------------
// نظام الحضور التلقائي الذكي (الأشهر والكبسولات)
// -------------------------------------------------------------------
...
function fetchAndBuildAttendanceButtons() {
    ...
    monthsContainer.innerHTML = `<div class="text-center p-15" style="grid-column: 1 / -1;">جاري جلب وأرشفة السجلات... ⏳</div>`;
    ...
    fetch(GAS_API_URL)
        ...
        .then(data => {
            ...
            if (!data || data.length === 0) {
                monthsContainer.innerHTML = `<div class="text-center p-15" style="grid-column: 1 / -1;">لا توجد سجلات حضور مؤرشفة حالياً.</div>`;
                return;
            }
            ...
            data.forEach((log, index) => {
                ...
                if (dateParts.length === 3) {
                    ...
                    if (!groupedAttendanceData[monthYearKey]) {
                        groupedAttendanceData[monthYearKey] = [];
                    }
...
// -------------------------------------------------------------------
// نظام عرض الملف الشخصي للعضو
// -------------------------------------------------------------------
function outputRowsHtml(list) {
    let html = "";
    list.forEach(row => {
        let displayVal = row.val;
        if (row.label === "أسماء الأبناء" || row.label === "الأبناء") {
            displayVal = cleanChildrenNames(row.val);
        }
        html += `
...
// دوال المنصات الاجتماعية والثيم
...
// -------------------------------------------------------------------
// نظام المعالجة والتحميل والإضافات للـ PDF
// -------------------------------------------------------------------
    ...
    const formatSection = (title, dataList) => {
        ...
        dataList.forEach(item => {
            if (item.label) {
                ...
                if (item.label === "أسماء الأبناء" || item.label === "الأبناء") {
                    displayVal = cleanChildrenNames(item.val);
                }
...
function copyMemberInfo() {
    ...
    const textToCopy = formatMemberDataAsText(currentActiveAlias);
    
    navigator.clipboard.writeText(textToCopy).then(() => {
        showNotification("تم نسخ معلومات العضو بالكامل 📋✅");
    }).catch(err => {
        showNotification("حدث خطأ أثناء النسخ ⚠️");
    });
}
...
function buildNameBoxesHtml(fullName) {
    ...
    let first = cleanParts[0] || "-";
    let father = cleanParts[1] || "-";
    let grand1 = cleanParts[2] || "-";
    let grand2 = cleanParts[3] || "-";
    let surname = cleanParts.slice(4).join(" ");
    ...
    return `
        <div style="display: flex; justify-content: space-between; gap: 8px; margin-bottom: 15px;">
            <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;">
                <div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الاسم الأول</div>
                <div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${first}</div>
            </div>
            <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;">
                <div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">اسم الأب</div>
                <div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${father}</div>
            </div>
            <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;">
                <div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الجد الأول</div>
                <div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${grand1}</div>
            </div>
            <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;">
                <div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الجد الثاني</div>
                <div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${grand2}</div>
            </div>
            <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;">
                <div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">اللقب</div>
                <div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${surname}</div>
            </div>
        </div>
    `;
}
...
function openPdfPreview() {
    ...
    // ستايل الإطار تم ضبطه بالمليمتر ليطابق ورقة A4 تماماً بدون انسكاب
    const frameStyle = `
        border: 2px solid #8c5531; 
        border-radius: 8px; 
        padding: 15px; 
        margin: 0 auto; 
    ...
    const buildBasicSection = (title, fullName, dataList) => {
        let secHtml = `
            <div class="pdf-section">
                <div class="pdf-section-title" style="word-spacing: 0;">${title}</div>
                ${buildNameBoxesHtml(fullName)}
                <div class="pdf-grid">
        `;
        dataList.forEach(item => {
            let displayVal = item.val;
        ...
        let secHtml = `
            <div class="pdf-section">
                ...
                <table class="pdf-table" style="font-size: 12px;">
                    <thead>
                        <tr><th>النوع</th><th>رقم الكتاب</th><th>التاريخ</th></tr>
                    </thead>
                    <tbody>
        `;
        dataList.forEach(item => {
            secHtml += `<tr><td>${item.title || '-'}</td><td>${item.doc || '-'}</td><td>${item.date || '-'}</td></tr>`;
        });
        secHtml += `</tbody></table></div>`;
        return secHtml;
    };

    let htmlContent = `<div id="printablePdfArea" style="width: 100%; direction: rtl; background-color: #fff;">`;

    // --- الصفحة الأولى ---
    htmlContent += `
        <div style="${frameStyle}">
            <div class="pdf-header-container" style="border-bottom: 2px solid #8c5531; padding-bottom: 10px; margin-bottom: 15px;">
                <div class="pdf-empty-right" style="width: 100px;"></div>
                <div class="pdf-logo-center" style="word-spacing: 0; text-align: center; flex: 1;">
                    <img src="https://i.ibb.co/N2BJ7Srb/N2BJ7Srb.jpg" alt="لوكو التجمع" style="width: 60px; height: 60px; border-radius: 10px;">
                    <h2 style="font-size: 18px; color: #8c5531; margin: 5px 0;">تجمع شباب الشريعة</h2>
                    <h3 style="font-size: 14px; color: #8c5531; margin: 0;">مديرية البصرة</h3>
                </div>
                <div class="pdf-avatar-left" style="width: 100px; text-align: left;">
                    <img src="${parseImgBBUrl(u.avatar)}" alt="صورة العضو" style="width: 100px; height: 100px; border-radius: 8px; object-fit: cover; border: 2px solid #8c5531;">
                </div>
            </div>
    `;

    htmlContent += buildBasicSection("المعلومات الأساسية", u.fullName, u.basic);
    htmlContent += buildPrintGrid("معلومات السكن", u.housing);
    htmlContent += `</div>`; 

    // الفاصل المخصص للورقة
    ...
    htmlContent += buildPrintTable("العمل والمسؤوليات", u.work);
    ...
    u.skills.forEach(skill => {
        if (skill.label === "دورة عسكرية" || skill.label === "عسكرية") {
            ...
            if (valStr.includes("نعم")) {
                ...
                if (numMatch) {
                    processedSkills.push({ label: "عدد الدورات", val: numMatch[0] });
                }
            } else {
                processedSkills.push({ label: "دورة عسكرية", val: "كلا" });
            }
        } else {
            processedSkills.push(skill);
        }
    });
    htmlContent += buildPrintGrid("الدورات والمهارات", processedSkills);
    
    htmlContent += buildPrintTable("كتب الشكر والتقدير", u.awards);
    htmlContent += buildPrintTable("السجل الانضباطي", u.punishments);
    
    htmlContent += `
            <div class="pdf-footer" style="position: absolute; bottom: 20px; left: 20px; right: 20px; border-top: 2px dashed #8c5531; padding-top: 10px;">
                <strong>مسؤول الموارد :</strong><span> محمد بشير الحلفي</span>
            </div>
        </div> 
    </div> 
    `;
...
function executePdfDownload() {
    ...
    showNotification("جاري تجهيز وتحميل الملف بدقة فائقة... ⏳");
