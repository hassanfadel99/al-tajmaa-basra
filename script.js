...
function loadSpecificDetail(key, element) {
    ...
    const headers = {
        ...
        'housing': 'معلومات السكن', // تم التعديل هنا
...
function copyMemberInfo() {
    ...
    navigator.clipboard.writeText(textToCopy).then(() => {
        showNotification("تم نسخ جميع معلومات الكادر بنجاح 📋✅");
    }).catch(err => {
...
function openPdfPreview() {
    ...
    // ستايل الإطار الإداري المضمن (لكل صفحة إطارها المستقل لتبدأ من الأعلى)
    const frameStyle = `
    ...
    const buildPrintGrid = (title, dataList) => {
        ...
        dataList.forEach(item => {
            ...
            secHtml += `<div class="pdf-grid-item"><strong>${item.label}</strong> <span>${displayVal}</span></div>`;
        });
        secHtml += `</div></div>`;
        return secHtml;
    };

    // دالة مساعدة لطباعة الجداول (كالعقوبات والشكر)
    ...
    let basicWithFullName = [{ label: "الاسم الكامل", val: u.fullName }, ...u.basic];
    htmlContent += buildPrintGrid("المعلومات الأساسية", basicWithFullName);
    htmlContent += buildPrintGrid("معلومات السكن", u.housing); // تم التعديل هنا
    htmlContent += `</div>`; // نهاية حاوية الصفحة الأولى

    // الفاصل الإجباري بين الصفحتين
    htmlContent += `<div class="html2pdf__page-break"></div>`;

    // 2. حاوية الصفحة الثانية (تبدأ مباشرة من أعلى الصفحة الجديدة)
    htmlContent += `
        <div style="${frameStyle}">
    `;
...
function executePdfDownload() {
    ...
    const opt = {
        ...
        // تم رفع الـ scale إلى 4 لضمان أعلى دقة ووضوح فائق (Razor-sharp) للطباعة
