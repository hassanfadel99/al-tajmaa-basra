...
// دالة ذكية لتصفية أسماء الأبناء (الاسم الأول فقط وفصلهم بحرف الواو)
...
// دالة لتجزئة الاسم الكامل وتلوينه بألوان هادئة وطافية (Matte)
...
// بناء المعاينة للـ PDF بشكل حاويات ثابتة لتكون صفحتين فقط بدقة
function openPdfPreview() {
    if (!currentActiveAlias) return;
    const u = MEMBERS_DATABASE[currentActiveAlias];
    
    // ستايل الإطار الإداري المضمن وإصلاح المسافات للخط العربي
    const frameStyle = `
    ...
    // دالة خاصة بقسم المعلومات الأساسية لدمج مربعات الاسم بداخلها
    ...
    // --- بداية الهيكل البرمجي للصفحتين ---
    let htmlContent = `<div id="printablePdfArea" style="width: 100%; direction: rtl; background-color: #fff;">`;

    // 1. حاوية الصفحة الأولى
    htmlContent += `
        <div style="${frameStyle}">
            <div class="pdf-header-container">
                <div class="pdf-empty-right"></div>
                <div class="pdf-logo-center" style="word-spacing: 0;">
                    <img src="https://i.ibb.co/N2BJ7Srb/N2BJ7Srb.jpg" alt="لوكو التجمع">
                    <h2>تجمع شباب الشريعة</h2>
                    <h3>مديرية البصرة</h3>
                </div>
                <div class="pdf-avatar-left">
                    <img src="${parseImgBBUrl(u.avatar)}" alt="صورة العضو">
                </div>
            </div>
    `;

    // دمج الاسم داخل المعلومات الأساسية وطباعتها
    htmlContent += buildBasicSection("المعلومات الأساسية", u.fullName, u.basic);
    htmlContent += buildPrintGrid("معلومات السكن", u.housing);
    htmlContent += `</div>`; // نهاية حاوية الصفحة الأولى

    // الفاصل الإجباري بين الصفحتين
    htmlContent += `<div class="html2pdf__page-break"></div>`;

    // 2. حاوية الصفحة الثانية
    htmlContent += `
        <div style="${frameStyle}">
    `;
    ...
    // التوقيع أسفل إطار الصفحة الثانية بثبات
    htmlContent += `
            <div class="pdf-footer" style="position: absolute; bottom: 20px; left: 20px; right: 20px;">
