/**
 * نظام المعالجة والتحقق البرمجي الرقمي المطور (الإصدار الماسي V11 - تصميم المجلد المفتوح)
...
// -------------------------------------------------------------------
// الدوال المساعدة والأدوات الأساسية
// -------------------------------------------------------------------
...
// -------------------------------------------------------------------
// دوال إدارة النوافذ المنبثقة للرموز السرية
// -------------------------------------------------------------------
...
// -------------------------------------------------------------------
// دوال عرض قائمة الإدارة والملف الشخصي
// -------------------------------------------------------------------
    ...
    let html = `
        <div class="open-folder-container">
            <div class="open-folder-header">
                <div class="folder-tab">📂 سجلات ${monthKey}</div>
            </div>
            <div class="open-folder-body">
                <div class="names-square-grid">
    `;

    const monthDays = groupedAttendanceData[monthKey].slice().reverse();
    monthDays.forEach(log => {
        html += `
            <button class="panel-btn saji-hover-card" style="border-radius: 50px; padding: 8px 10px; display: flex; flex-direction: column; align-items: center; justify-content: center; line-height: 1.4;" onclick="openAttendanceDetailModal(${log.originalIndex})">
                <strong style="font-size: 1rem;">${log.day}</strong>
                <span style="font-size: 0.8rem; color: var(--gold); letter-spacing: 1px;">${log.cleanDate}</span>
            </button>
        `;
    });
...
// -------------------------------------------------------------------
// أرشيف الاستمارات (المجلدات والتحميل الشامل والـ PDF)
// -------------------------------------------------------------------
...
function renderFormsCapsules(monthKey, type) {
    ...
    document.getElementById('monthActionContainer').innerHTML = ``; // تم نقله لداخل المجلد

    const container = document.getElementById('formsCapsulesContainer');
    container.className = "mt-10"; // إزالة الكلاس القديم لمنع تكسر الشبكة
    
    let html = `
        <div class="open-folder-container">
            <div class="open-folder-header">
                <div class="folder-tab">📂 أنشطة ${monthKey}</div>
            </div>
            <div class="open-folder-body">
    ...
    monthData.forEach(item => {
        html += `
            <button class="panel-btn saji-hover-card" style="border-radius: 50px; padding: 8px 10px; display: flex; flex-direction: column; align-items: center;" onclick="openFormTextModal('${type}', ${item.originalIndex}, '${item.cleanDate}')">
...
function buildFormPageHtml(form, type) {
    ...
    return `
        <div style="width: 210mm; height: 296mm; background: #fff; padding: 15mm; box-sizing: border-box; font-family: Arial, sans-serif; direction: rtl; position:relative; overflow: hidden; margin: 0; color: #333;">
...
function generateFormPDF() {
    ...
    document.getElementById('formPdfContentArea').innerHTML = `<div id="printableFormArea" style="background:transparent; direction: ltr; width: 210mm; position: relative;">${htmlPage}</div>`;
...
function generateMonthFormsPDF(monthKey, type) {
    ...
    document.getElementById('formPdfContentArea').innerHTML = `<div id="printableFormArea" style="background:transparent; direction: ltr; width: 210mm; position: relative;">${finalHtml}</div>`;
    document.getElementById('formPdfModal').classList.remove('hidden');
...
// -------------------------------------------------------------------
// دوال الملف الشخصي وطباعته (صفحتين)
// -------------------------------------------------------------------
...
function openPdfPreview() {
    ...
    const frameStyle = `width: 210mm; height: 296mm; background: #fff; padding: 15mm; box-sizing: border-box; font-family: Arial, sans-serif; direction: rtl; position: relative; overflow: hidden; margin: 0; color: #333;`;
    ...
    let htmlContent = `
        <div id="printableProfileArea" style="background:transparent; direction: ltr; width: 210mm; position: relative;">
            ${page1}
