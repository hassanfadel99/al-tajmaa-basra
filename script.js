/**
 * نظام المعالجة والتحقق البرمجي الرقمي المطور (الإصدار الماسي المضبوط)
 * تجمع شباب الشريعة-مديرية البصرة
 */

// الروابط الأصلية المرتبطة بجداولك المباشرة
...
// قاعدة بيانات الأعضاء بالأسماء الصريحة
...
function openSidebarSection(sectionType) {
    toggleSidebar(false);
    document.getElementById('dashboardScreen').classList.add('hidden');
    if (sectionType === 'settings') document.getElementById('settingsView').classList.remove('hidden');
    else if (sectionType === 'about') document.getElementById('aboutView').classList.remove('hidden');
}
...
function goBackToDashboard() {
    const views = ['adminScreen', 'activitiesScreen', 'socialScreen', 'adminKaderListScreen', 'attendanceLogsScreen', 'formsCategoryScreen', 'formsMonthsScreen', 'formsListScreen'];
    views.forEach(v => {
...
function submitDirectCode() {
    ...
    } else document.getElementById('directModalCodeError').classList.remove('hidden');
}
...
function submitAdminPassword() {
    ...
    } else document.getElementById('modalAdminError').classList.remove('hidden');
}
...
function openAttendanceDetailModal(originalIndex) {
    ...
    if (!log) return;
    let cleanDateStr = String(log.date).split('T')[0];
    document.getElementById('attendanceModalTitle').innerHTML = `سجل حضور يوم ${log.day} <br><span style="font-size:1rem; color:#fff;">(${cleanDateStr})</span>`;
...
// -------------------------------------------------------------------
// أرشيف الاستمارات (المجلدات والكبسولات الذكية)
// -------------------------------------------------------------------
...
function renderFormsCapsules(monthKey, type) {
    ...
    monthData.forEach(item => {
        ...
        btn.innerHTML = `
            ...
            <span style="font-size: 0.85rem; color: #8c5531; margin-top:5px; font-weight:bold;">${item.cleanDate}</span>
        `;
...
function openFormTextModal(type, index, dateVal) {
    ...
    document.getElementById('formModalTitle').innerHTML = `استمارة نشاط رقمية<br><span style="font-size:0.9rem;">(${dateVal})</span>`;
    ...
    for(let key in form) {
        ...
        let shouldExclude = excludeWords.some(word => lowKey.includes(word));
        ...
        if(!key.includes("Timestamp") && !key.includes("طابع زمني")) {
            ...
            textHtml += `<div style="margin-bottom:8px; border-bottom:1px solid rgba(0,0,0,0.05); padding-bottom:5px;">
...
// -------------------------------------------------------------------
// محاكي بناء الـ PDF الخلاب الماسي (التصميم الكبسولي المعتمد)
// -------------------------------------------------------------------
function generateFormPDF() {
    ...
    let headerText = type === 'tajammu' 
        ? '<div style="display: inline-block; text-align: center;"><div style="font-size: 17px; font-weight: bold; white-space: nowrap;">تجمع شباب الشريعة</div><div style="font-size: 14px; font-weight: normal; margin-top:2px;">مديرية البصرة</div></div>' 
        : '<div style="display: inline-block; text-align: center;"><div style="font-size: 16px; font-weight: bold; white-space: nowrap;">جمعية كشافة الامام الحسين (ع)</div><div style="font-size: 14px; font-weight: normal; margin-top:2px;">مفوضية البصرة</div></div>';
    ...
    for(let key in form) {
        ...
        let val = form[key];
        // إذا كان المستخدم ترك الحقل فارغاً، نكتب "لا يوجد"
        if (val === undefined || val === null || String(val).trim() === "") { val = "لا يوجد"; }

        if(!key.includes("Timestamp") && !key.includes("طابع زمني")) {
            
            // قصر التمدد على (أسماء المشاركين) فقط
            let isFullWidth = key.includes("اسماء") || key.includes("أسماء");
            let itemWidth = isFullWidth ? "100%" : "48%";
            let labelWidth = isFullWidth ? "20%" : "35%";
            let valueWidth = isFullWidth ? "80%" : "65%";
            
            let displayValue = String(val).trim();

            // الفلتر الذكي للأسماء (مسح الفوارز والأسطر الجديدة، ووضع الأسماء ككتل بجانب بعضها)
            if (isFullWidth) {
            ...
            // التصميم الكبسولي (العنوان في اليمين والقيمة في اليسار) بـ Flexbox RTL
            gridItems += `
                <div style="width: ${itemWidth}; margin-bottom: 15px; border: 1.5px solid ${themeColor}; border-radius: 8px; overflow: hidden; box-shadow: 1px 1px 3px rgba(0,0,0,0.05); background-color: #fff; page-break-inside: avoid;">
                    <div style="display: flex; direction: rtl; min-height: 35px; width: 100%;">
                        <div style="width: ${labelWidth}; background-color: ${themeColor}; color: #fff; padding: 10px; font-weight: bold; text-align: right; font-size: 13px; display: flex; align-items: center; border-left: 1.5px solid ${themeColor}; box-sizing: border-box;">
                            ${key}
                        </div>
                        <div style="width: ${valueWidth}; padding: 10px; font-size: 13px; color: #222; font-weight: bold; text-align: right; line-height: 1.8; display: flex; align-items: center; flex-wrap: wrap; box-sizing: border-box;">
                            ${displayValue}
                        </div>
                    </div>
                </div>
    ...
    const htmlContent = `
        <div id="printableFormArea" style="width: 210mm; min-height: 297mm; background: #fff; padding: 15mm; box-sizing: border-box; font-family: Arial, sans-serif; direction: rtl; position:relative; overflow: hidden;">
            ...
            <div style="position: relative; z-index: 2; padding: 5mm; display: flex; flex-direction: column; height: 100%; min-height: 250mm;">
                ...
                <table style="width: 100%; border: none; margin-bottom: 25px; border-bottom: 2px solid ${themeColor}; padding-bottom: 15px;">
                    <tr>
                        ...
                        <td style="width: 34%; text-align: center; vertical-align: middle;">
                            <div style="display: inline-block; border: 2px solid ${themeColor}; border-radius: 25px; padding: 6px 18px; color: ${themeColor}; font-weight: bold; font-size: 17px; background: #fff; box-shadow: 0px 3px 0px rgba(0,0,0,0.05);">
                                ${formTitle}
                            </div>
                        </td>
                ...
                <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-content: flex-start; width: 100%; direction: rtl;">
                    ${gridItems}
                </div>

                <table style="width: 100%; border: none; margin-top: auto; border-top: 1px solid #ccc; padding-top: 15px; font-weight: bold; text-align: center; color: #333;">
                    <tr>
    ...
    document.getElementById('formPdfContentArea').innerHTML = htmlContent;
...
function executeFormPdfDownload() {
    ...
    const typeName = currentFormView.type === 'tajammu' ? 'التجمع' : 'الكشافة';
    const fileName = `استمارة_نشاطات_${typeName}.pdf`;
    ...
    showNotification("جاري تصدير الاستمارة بجودة إدارية... ⏳");
    html2pdf().set(opt).from(element).save().then(() => {
        closeFormPdfModal();
        showNotification("تم تحميل الاستمارة الرسمية بنجاح ✅");
    });
}

// -------------------------------------------------------------------
// دوال الملف الشخصي والمظهر العام
// -------------------------------------------------------------------
...
function closeSocialModal() { document.getElementById('socialLinksModal').classList.add('hidden'); }

function toggleSystemTheme() {
    const isLightMode = document.body.classList.toggle('light-mode');
    if(isLightMode) showNotification("تم تفعيل الوضع النهاري (صاج فاتح) ☀️");
    else showNotification("تم تفعيل الوضع الليلي (صاج داكن) 🌙");
}
...
function buildNameBoxesHtml(fullName) {
    ...
    return `<div style="display: flex; justify-content: space-between; gap: 8px; margin-bottom: 15px;">
        <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;"><div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الاسم الأول</div><div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${first}</div></div>
        <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;"><div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">اسم الأب</div><div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${father}</div></div>
        <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;"><div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الجد الأول</div><div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${grand1}</div></div>
        <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;"><div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">الجد الثاني</div><div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${grand2}</div></div>
        <div style="flex: 1; border: 1px solid #d3c4b1; border-radius: 6px; overflow: hidden; text-align: center;"><div style="background-color: #e8dcc8; color: #5a3a22; font-size: 11px; padding: 5px; font-weight: bold;">اللقب</div><div style="background-color: #fcf9f5; color: #333; font-size: 13px; font-weight: bold; padding: 8px;">${surname}</div></div>
    </div>`;
}
...
function openPdfPreview() {
    if (!currentActiveAlias) return;
    const u = MEMBERS_DATABASE[currentActiveAlias];
    const frameStyle = `border: 2px solid #8c5531; border-radius: 8px; padding: 15px; margin: 0 auto; background-color: #fff; box-sizing: border-box; height: 277mm; width: 100%; position: relative; font-family: Arial, sans-serif; word-spacing: 3px; line-height: 1.6; overflow: hidden;`;

    const buildBasicSection = (title, fullName, dataList) => {
        let secHtml = `<div class="pdf-section"><div class="pdf-section-title" style="word-spacing: 0;">${title}</div>${buildNameBoxesHtml(fullName)}<div class="pdf-grid">`;
        dataList.forEach(item => {
            let displayVal = item.val;
            if (item.label === "أسماء الأبناء" || item.label === "الأبناء") displayVal = cleanChildrenNames(item.val);
            secHtml += `<div class="pdf-grid-item" style="padding: 10px 12px;"><strong>${item.label}</strong> <span>${displayVal}</span></div>`;
        });
    ...
    const buildPrintGrid = (title, dataList) => {
        if (!dataList || dataList.length === 0) return "";
        let secHtml = `<div class="pdf-section"><div class="pdf-section-title" style="word-spacing: 0;">${title}</div><div class="pdf-grid">`;
        dataList.forEach(item => {
            let displayVal = item.val;
            if (item.label === "أسماء الأبناء" || item.label === "الأبناء") displayVal = cleanChildrenNames(item.val);
            secHtml += `<div class="pdf-grid-item" style="padding: 10px 12px;"><strong>${item.label}</strong> <span>${displayVal}</span></div>`;
    ...
    const buildPrintTable = (title, dataList) => {
        if (!dataList || dataList.length === 0) return "";
        let secHtml = `<div class="pdf-section"><div class="pdf-section-title" style="word-spacing: 0;">${title}</div><table class="pdf-table" style="font-size: 12px;"><thead><tr><th>النوع</th><th>رقم الكتاب</th><th>التاريخ</th></tr></thead><tbody>`;
        dataList.forEach(item => { secHtml += `<tr><td>${item.title || '-'}</td><td>${item.doc || '-'}</td><td>${item.date || '-'}</td></tr>`; });
        return secHtml + `</tbody></table></div>`;
    };

    let htmlContent = `<div id="printablePdfArea" style="width: 100%; direction: rtl; background-color: #fff;"><div style="${frameStyle}"><div class="pdf-header-container" style="border-bottom: 2px solid #8c5531; padding-bottom: 10px; margin-bottom: 15px;"><div style="width: 100px;"></div><div style="text-align: center; flex: 1;"><img src="https://i.ibb.co/N2BJ7Srb/N2BJ7Srb.jpg" style="width: 60px; height: 60px; border-radius: 10px;"><h2 style="font-size: 18px; color: #8c5531; margin: 5px 0;">تجمع شباب الشريعة</h2><h3 style="font-size: 14px; color: #8c5531; margin: 0;">مديرية البصرة</h3></div><div style="width: 100px; text-align: left;"><img src="${parseImgBBUrl(u.avatar)}" style="width: 100px; height: 100px; border-radius: 8px; object-fit: cover; border: 2px solid #8c5531;"></div></div>${buildBasicSection("المعلومات الأساسية", u.fullName, u.basic)}${buildPrintGrid("معلومات السكن", u.housing)}</div><div class="html2pdf__page-break"></div><div style="${frameStyle}">${buildPrintTable("العمل والمسؤوليات", u.work)}`;
    ...
    htmlContent += `${buildPrintGrid("الدورات والمهارات", processedSkills)}${buildPrintTable("كتب الشكر والتقدير", u.awards)}${buildPrintTable("السجل الانضباطي", u.punishments)}<div style="position: absolute; bottom: 20px; left: 20px; right: 20px; border-top: 2px dashed #8c5531; padding-top: 10px;"><strong>مسؤول الموارد :</strong><span> محمد بشير الحلفي</span></div></div></div>`;
...
function executePdfDownload() {
    const element = document.getElementById('printablePdfArea');
    const u = MEMBERS_DATABASE[currentActiveAlias];
    const fileName = `ملف_معلومات_${u.fullName.replace(/\s+/g, '_')}.pdf`;
    const opt = { margin: [5, 5, 5, 5], filename: fileName, image: { type: 'jpeg', quality: 1 }, html2canvas: { scale: 3, useCORS: true, scrollY: 0 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }, pagebreak: { mode: 'css', before: '.html2pdf__page-break' } };
    showNotification("جاري تصدير الملف بجودة إدارية فائقة... ⏳");
    html2pdf().set(opt).from(element).save().then(() => { closePdfPreview(); showNotification("تم تحميل الملف بنجاح ✅"); });
}
