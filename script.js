/**
 * نظام المعالجة والتحقق البرمجي الرقمي المطور (الإصدار الماسي للطباعة والتمدد اللوني)
...
// -------------------------------------------------------------------
// محاكي بناء الـ PDF الخلاب الماسي
// -------------------------------------------------------------------
function generateFormPDF() {
    ...
    for(let key in form) {
        ...
        if(!key.includes("Timestamp") && !key.includes("طابع زمني")) {
            ...
            // قصر التمدد العرضي حصرياً على حقل الأسماء والمشاركين
            ...
            // استخدام flex لتمدد اللون بشكل صحيح ومحاذاة المربعات جنباً إلى جنب
            gridItems += `
                <div style="width: ${itemWidth}; display: flex; border: 1.5px solid ${themeColor}; border-radius: 8px; overflow: hidden; box-shadow: 1px 1px 3px rgba(0,0,0,0.05); box-sizing: border-box; background-color: #fff; page-break-inside: avoid;">
                    <div style="display: flex; direction: rtl; width: 100%; align-items: stretch;">
...
function executeFormPdfDownload() {
    ...
    // إضافة ختم زمني لمنع متصفحات الهاتف من حظر التحميل
    const uniqueId = new Date().getTime();
    const fileName = `استمارة_نشاطات_${typeName}_${uniqueId}.pdf`;
    ...
    showNotification("جاري تصدير الاستمارة، يرجى الانتظار... ⏳");
    ...
    html2pdf().set(opt).from(element).save().then(() => {
        ...
        console.error(err);
        showNotification("حدث خطأ أثناء التحميل، جرب مجدداً ⚠️");
    });
}
