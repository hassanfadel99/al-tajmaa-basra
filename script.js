/**
 * نظام المعالجة والتحقق البرمجي الرقمي المطور (الإصدار الماسي V35 - محرك الطباعة التتابعي والنسخ الإجباري)
...
// -------------------------------------------------------------------
// الإتاحة العامة (Global Window Mapping) للبيئة المعزولة
// -------------------------------------------------------------------
...
window.buildAdminKaderButtons = function() {
    ...
    Object.keys(MEMBERS_DATABASE).forEach(alias => {
        ...
        b.innerText = alias;
        b.onclick = () => { window.currentActiveAlias = alias; window.displayMemberCardData(alias); };
...
window.openFormTextModal = function(type, index, dateVal) {
    ...
    if (type === 'planning') {
        ...
        for(let key in form) {
            ...
            // التعديل القاطع: إذا كانت القيمة فارغة أو تحتوي على جملة "لا يوجد" القادمة من قاعدة البيانات، نقوم بمسحها تماماً
...
// ============================================================================
// PDF التخطيط (تقطيع ذكي: 4 برامج لكل صفحة مع حماية من الأوراق البيضاء)
// ============================================================================
    ...
    if (isLast) {
        ...
        <div style="position: absolute; bottom: 12mm; left: 12mm; right: 12mm; border-top: 1.5px dashed ${themeColor}; padding-top: 8px; display: flex; justify-content: space-between; font-size: 13px; color: black; font-weight: bold; text-align: center; z-index: 2;">
            ...
            <div style="width: 45%;">
                ...
                <div style="margin-top: 20px; color: ${themeColor}; font-size: 14px;">وليد خالد الحيدري</div>
            </div>
        </div>`;
    }

    // تقليص الارتفاع إلى 294mm لضمان عدم فيضان أي بكسل للورقة البيضاء
    return `
        <div class="pdf-print-page" style="width: 210mm; height: 294mm; max-height: 294mm; box-sizing: border-box; font-family: Arial, sans-serif; direction: rtl; margin: 0 auto; color: #333; background: #fff; position: relative; padding: 12mm; overflow: hidden;">
...
window.buildFormPageHtml = function(form, type) {
    ...
    for(let key in form) {
        ...
        // التعديل القاطع هنا للـ PDF: البحث عن "لا يوجد" ومسحها
        if (val === undefined || val === null || String(val).trim() === "" || String(val).trim() === "لا يوجد") { 
            val = ""; 
        }
        ...
        if(!key.includes("Timestamp") && !key.includes("طابع زمني")) {
            ...
            if (isFullWidth) {
                ...
                if (namesArray.length > 0) {
                    ...
                    displayValue = namesArray.map(n => `<span style="display: inline-block; white-space: nowrap; margin-left: 12px;">${n}</span>`).join(''); 
                }
            } else { 
            ...
            gridItems += `
                <div style="width: ${itemWidth}; display: flex; flex-direction: column; border: 1.5px solid ${themeColor}; border-radius: 8px; overflow: hidden; box-shadow: 1px 1px 3px rgba(0,0,0,0.05); box-sizing: border-box; background-color: #fff; page-break-inside: avoid; margin-bottom: 12px;">
                    <div style="display: flex; direction: rtl; width: 100%; flex-grow: 1; align-items: stretch;">
                        <div style="width: ${labelWidth}; background-color: ${themeColor}; color: #fff; padding: ${customPadding}; font-weight: bold; text-align: right; font-size: ${customFontSize}; display: flex; align-items: center; border-left: 1.5px solid ${themeColor}; box-sizing: border-box;">${key}</div>
                        <div style="width: ${valueWidth}; padding: ${customPadding}; font-size: ${customFontSize}; color: #222; font-weight: bold; text-align: right; line-height: ${customLineHeight}; display: flex; align-items: center; justify-content: flex-start; box-sizing: border-box; flex-wrap: wrap;">${displayValue}</div>
                    </div>
                </div>`;
        }
    }
    ...
    return `
        <div class="pdf-print-page" style="width: 210mm; height: 294mm; max-height: 294mm; display: block; background: #fff; padding: 15mm; padding-bottom: 35mm; box-sizing: border-box; font-family: Arial, sans-serif; direction: rtl; position:relative; margin: 0 auto; color: #333; overflow: hidden;">
            ...
            <div style="position: relative; z-index: 2; height: 100%;">
                <table style="width: 100%; border: none; margin-bottom: 20px; border-bottom: 2px solid ${themeColor}; padding-bottom: 15px;">
                    <tr>
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 12px; justify-content: flex-start; align-content: flex-start; width: 100%; direction: rtl;">
                    ${gridItems}
                </div>
            </div>

            <div style="position: absolute; bottom: 12mm; left: 15mm; right: 15mm; border-top: 1px solid #ccc; padding-top: 12px; font-weight: bold; text-align: center; color: #333; direction: rtl; display: flex; justify-content: space-between; z-index: 2;">
...
// -------------------------------------------------------------------
// وظائف معالجة الـ PDF (نظام شريط التحميل والطباعة التتابعية)
// -------------------------------------------------------------------
...
window.generateMonthFormsPDF = function(monthKey, type) {
    ...
    // تم إلغاء كلاس الفواصل تماماً، فقط نضع الاستمارات وراء بعض والمحرك سيتكفل بالباقي
...
window.executeFormPdfDownload = async function() {
    ...
    setTimeout(async () => {
        try {
            ...
            if (pages.length > 0) {
                // النظام التتابعي: يطبع الصفحة الأولى ثم يضيف الصفحات الأخرى تباعاً (يمنع الصفحة البيضاء)
...
// ============================================================================
// نظام النسخ الإجباري للمعلومات (يعمل في كل المنصات)
// ============================================================================
...
// دالة قوة إجبارية للنسخ (Fallback)
